...
DEFAULT_LAMBDAS_GENERIC = [0.0, 1e-8, 1e-6, 1e-4, 1e-2]
DEFAULT_LAMBDAS_BURGERS = [0.0, 1e-4, 1e-6, 1e-8]
...
def run_one(cfg: RunConfig, run_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        # Normalize coords for sirenmlp training stability
...
def main() -> None:
    ...
    p.add_argument("--tv_lambdas", nargs="*", type=float, default=None)
    ...
    if not args.tv_lambdas:
        if str(args.dataset).lower() == "burgers":
            args.tv_lambdas = list(DEFAULT_LAMBDAS_BURGERS)
        else:
            args.tv_lambdas = list(DEFAULT_LAMBDAS_GENERIC)

    all_rows: list[dict[str, Any]] = []

    for tv_type in args.tv_types:
        tv_type = str(tv_type)
        root = Path("run_results") / "tv_lambda_sweep" / str(args.dataset) / str(tv_type)
    ...
    if (not args.dry_run) and all_rows:
        out_path = Path("run_results") / "tv_lambda_sweep" / str(args.dataset) / "all_results.csv"
