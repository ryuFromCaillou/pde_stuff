...
def run_one_dataset(cfg: RunConfig) -> list[dict[str, Any]]:
    ...
    base_dir = Path("run_results") / "derivative_compare" / cfg.dataset / f"seed_{cfg.seed:03d}"
...
def main():
    ...
    for dataset in args.datasets:
        ...
        ds_out = Path("run_results") / "derivative_compare" / str(dataset) / "summary.csv"
        if dataset_rows:
            _write_csv(ds_out, dataset_rows, SUMMARY_FIELDS)
            print(f"Wrote {ds_out}")

    out_path = Path("run_results") / "derivative_compare" / "all_results.csv"
    if all_rows:
