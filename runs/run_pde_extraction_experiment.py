...
def _freeze_model(m: torch.nn.Module) -> None:
    m.eval()
...
def run_one_dataset(cfg: RunConfig) -> list[dict[str, Any]]:
    ...
    base_dir = Path("run_results") / "pde_extraction" / str(cfg.dataset) / f"seed_{cfg.seed:03d}"
    ...
    for model_name, base_model in models.items():
        ...
        _freeze_model(base_model)
        phys_model = PhysCoordWrapper(base_model, a_t=a_t, b_t=b_t, a_x=a_x, b_x=b_x).to(cfg.device)

        feature_terms, _ = true_coeffs_for_dataset(cfg.dataset, cfg)

        # Least-squares extraction (FeatureTensor(normalize=False) inside extract_pde_ls)
        res = extract_pde_ls(
            phys_model,
            t_train,
            x_train,
        ...
        coeffs = np.asarray(res["coeffs"], dtype=float).reshape(-1)

        true_names, true_vec = true_coeffs_for_dataset(cfg.dataset, cfg)
        true_map = {n: float(c) for n, c in zip(true_names, true_vec)}
        true_coeffs = np.array([true_map[n] for n in names], dtype=float)

        abs_err = np.abs(coeffs - true_coeffs)
        l2_err = float(np.linalg.norm(coeffs - true_coeffs))
        ...
        metrics = {
            ...
            "abs_coeff_error": abs_err.tolist(),
            "l2_coeff_error": l2_err,
            "residuals": np.asarray(res["residuals"], dtype=float).reshape(-1).tolist(),
            "rank": int(res["rank"]),
            "singular_values": np.asarray(res["singular_values"], dtype=float).reshape(-1).tolist(),
        ...
        dataset_rows.append(
            {
                ...
                "coeffs": json.dumps(metrics["coeffs"]),
                "true_coeffs": json.dumps(metrics["true_coeffs"]),
                "abs_coeff_error": json.dumps(metrics["abs_coeff_error"]),
                "l2_coeff_error": float(metrics["l2_coeff_error"]),
                "residuals": json.dumps(metrics["residuals"]),
                "rank": int(metrics["rank"]),
                "singular_values": json.dumps(metrics["singular_values"]),
...
def main() -> None:
    ...
    for dataset in args.datasets:
        ...
        ds_summary = Path("run_results") / "pde_extraction" / dataset / "summary.csv"
        if by_dataset[dataset]:
            _write_csv(ds_summary, by_dataset[dataset], SUMMARY_FIELDS)
            print(f"Wrote {ds_summary}")

    out_path = Path("run_results") / "pde_extraction" / "all_results.csv"
    if all_rows:
