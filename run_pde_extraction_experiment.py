...
def run_one_dataset(cfg: RunConfig) -> list[dict[str, Any]]:
    ...
    base_dir = Path("runs") / "pde_extraction" / str(cfg.dataset) / f"seed_{cfg.seed:03d}"
...
def main() -> None:
    ...
    for dataset in args.datasets:
        ...
        ds_summary = Path("runs") / "pde_extraction" / dataset / "summary.csv"
        if by_dataset[dataset]:
            _write_csv(ds_summary, by_dataset[dataset], SUMMARY_FIELDS)
            print(f"Wrote {ds_summary}")

    out_path = Path("runs") / "pde_extraction" / "all_results.csv"
    if all_rows:
